...
日本語を明晰にするための原則に基づき、以下の6つの診断基準で文章を評価してください。
...
【「の」の連続の数え方（重要）】
「の」の連続は、助詞や句読点で区切られていない、ひとつながりの名詞連結の中でのみ数える。
「は」「が」「を」「に」「へ」「と」「も」等の助詞、または読点・句点で区切られた場合、そこで連続は途切れる（別々に数える）。
- 数える例：「上司の指示の意図の理解」→ ひとつながりで「の」が3連続（指摘対象）
- 数えない例：「政治家の責任の取り方は自らの進退だ」→「政治家の責任の取り方」で「の」は2連続、「自らの進退」は「は」を挟んだ別の連結で1つ。連続していないので3連続ではない（指摘対象外）
必ず、助詞・句読点をまたいで「の」を合算しないこと。

【修正例を出すときの自己検証】
「の」を減らす修正例を提示する場合は、修正後の表現で実際に「の」の数が減っているか確認してから出力すること。減っていない修正例（例:「政治家にとっての責任の取り方」は元と同じ「の」2つで無効）は提示しない。
...
- commentには、実際に問題があると判断した箇所のみを記述する
- 「この箇所は問題なし」「〜は特に問題ありません」のように、検討した結果問題がなかった箇所への言及をcommentに含めてはならない。問題がない箇所は一切書かず、沈黙する
- 基準内に問題が一つも見つからない場合はstatusを「OK」とし、commentは「問題ありません」のみの簡潔な総括（1文程度）とする。問題箇所ごとの列挙は行わない
- 基準内に複数の問題箇所がある場合も、問題がある箇所のみを【問題箇所】【理由】【修正例】の形式で列挙する。問題の有無を箇所ごとに逐一報告するのではなく、問題がある箇所だけを取り上げる
...
### 8. 連鎖的な同一構文の抑制
- 「AがB。BがC。CがD。」のように、前文の述語や主題を次文の頭で繰り返す連鎖（しりとり式）を3文以上続けない
- 因果関係が連鎖する場合は、1文にまとめるか、「〜し、それによって〜」等の接続表現で簡潔に統合する
- 悪い例：「本を読まなければ、語彙力が落ちます。語彙力が落ちれば、思考力も落ちます。思考力が落ちれば、様々な問題が生じます。」
- 良い例：「本を読まなければ語彙力が落ち、ひいては思考力の低下にもつながります。」
`;
...
export async function POST(req: NextRequest) {
  try {
    // ログイン中ユーザーの取得（現時点では利用制限等には使わず、未ログインでも従来通り動作する）
